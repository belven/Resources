...
public class ItemStackFunctions {
	...
	static {
		...
		// Much thanks to @DPOHVAR for sharing the PowerNBT code that powers the
		// reflection approach.
		try {
	...
	/*
	 * public static ItemStack stringToItemStack(final String data, final String
	 * name) { ItemStack inventory = null;
	 * 
	 * try { final ByteArrayInputStream inputStream = new ByteArrayInputStream(
	 * new BigInteger(data, 32).toByteArray());
	 * 
	 * // More MC internals :( Method loadMethod = class_NBTBase.getMethod("a",
	 * DataInput.class); final Object itemList = loadMethod.invoke(null, new
	 * DataInputStream(inputStream));
	 * 
	 * Method sizeMethod = class_NBTTagList.getMethod("size"); Method getMethod
	 * = class_NBTTagList.getMethod("get", Integer.TYPE); final int listSize =
	 * (Integer) sizeMethod.invoke(itemList);
	 * 
	 * Method isEmptyMethod = class_NBTTagCompound.getMethod("isEmpty");
	 * 
	 * Constructor<?> inventoryConstructor = class_CraftInventoryCustom
	 * .getConstructor(InventoryHolder.class, Integer.TYPE, String.class);
	 * inventory = (Inventory) inventoryConstructor .newInstance(null, listSize,
	 * ChatColor.translateAlternateColorCodes('&', name));
	 * 
	 * Method setItemMethod = class_CraftInventoryCustom.getMethod( "setItem",
	 * Integer.TYPE, ItemStack.class);
	 * 
	 * for (int i = 0; i < listSize; i++) { final Object inputObject =
	 * getMethod.invoke(itemList, i); if (!(Boolean)
	 * isEmptyMethod.invoke(inputObject)) { Method createMethod =
	 * class_ItemStack.getMethod( "createStack", inputObject.getClass()); Object
	 * newStack = createMethod.invoke(null, inputObject); Method
	 * bukkitCopyMethod = class_CraftItemStack.getMethod( "asBukkitCopy",
	 * class_ItemStack); Object newCraftStack = bukkitCopyMethod.invoke(null,
	 * newStack); setItemMethod.invoke(inventory, i, newCraftStack); } } } catch
	 * (Throwable ex) { ex.printStackTrace(); } return inventory; }
	 */
	...
	public static Inventory stringToInventory(final String data, final String name) {
		...
		try {
			...
			Constructor<?> inventoryConstructor = class_CraftInventoryCustom.getConstructor(InventoryHolder.class,
